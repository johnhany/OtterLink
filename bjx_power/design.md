...
**路径原则：所有文件读写都在用户目录（`~/bjx/`）下进行**，不依赖 `/opt`、`/tmp`，避免文件权限问题；本地开发同时支持 Windows 与 macOS。
...
| `references/REBUILD.md` | 全灭重建手册（含 WAF 绕过参数） | 保留作灾备参考（沙箱环境专用，其路径约定与本文无关） |
...
```
                        ┌──────────────────────────── 腾讯云香港 CVM ───────────────────────────┐
                        │                                                                       │
  北极星电力网 ──抓取──▶ │  bjx-daily.timer (systemd 用户级, 周一至周五 08:30 北京时间)            │
  (bjx.com.cn, WAF)     │       │                                                               │
                        │       ▼                                                               │
                        ...
                        │       │                                                               │
                        │       ▼                                                               │
                        │  ~/bjx/site/  ◀── Nginx (443, HTTPS) ──▶ https://bjx.geekbit.org      │
                        │                                                                       │
                        └───────────────────────────────────────────────────────────────────────┘
```
...
- **systemd 用户级 timer**（`systemctl --user`）而非 cron 或系统级 service：代码、数据、站点全部在登录用户家目录，服务以该用户身份运行，**全程不需要 root/chown**（仅 nginx 安装与 `loginctl enable-linger` 需要 sudo）；日志走 `journalctl --user`，支持 `Persistent=true` 补跑。
- HTTPS 用 certbot + Let's Encrypt 自动签发续期。

### 2.2 每日工作流程（全自动，无人介入）

1. **触发**：systemd 用户级 timer 于北京时间周一至周五 08:30 触发 `bjx-daily.service`。
...
4. **对外服务**：Nginx 静态服务 `~/bjx/site/`，报告生成即上线，无需重启任何服务。
5. **收尾（可选）**：`pack.py export` 导出状态快照作本地备份。

### 2.3 服务器目录规划（全部在用户目录下）

```
~/bjx/                      # 项目根（代码 + 数据 + 站点，均无需 root）
├── crawler.py  metrics.py  pack.py  site.py  run_daily.sh
├── requirements.txt
├── config/columns.json
├── FORMAT.md  REBUILD.md
├── deploy/                   # nginx 配置、systemd 用户级 unit 模板
├── .venv/                    # Python 虚拟环境
├── data/                     # $BJX_BASE（默认 ~/bjx/data）：运行数据
│   ├── config/columns.json   # 首次部署时从代码目录复制
│   ├── articles/YYYY-MM/     # 文章全文 + images/
│   ├── briefing/YYYY-MM-DD.md
│   ├── metrics/  logs/  state/
├── site/                     # $BJX_SITE（默认 ~/bjx/site）：静态站点（Nginx root）
│   ├── index.html
│   ├── report/YYYY-MM-DD.html
│   └── assets/style.css
└── backup/                   # $BJX_OUT（默认 ~/bjx/backup）：pack.py 导出
```

本地开发（Windows/macOS）同理：所有路径默认也在 `~/bjx/` 下，或用环境变量指向项目内目录（见 README 本地部署）。
...
1. **BASE 路径环境变量化，默认落在用户目录**（第 16 行）：
   ...
   `CONF/STATE_DIR/ART_DIR/BRIEF_DIR/LOG_DIR` 均由 BASE 派生，无需逐个改。`main()` 里的 `os.chdir(BASE)` 保持不变。Windows 下 `~` 解析为 `C:\Users\<用户>`，本地与服务器行为一致。
...
### 3.2 `metrics.py`（2 处改动，定位调整为可选工具）

- 同样改 `BASE = os.environ.get("BJX_BASE") or os.path.expanduser(os.path.join("~", "bjx", "data"))`。
- 索引拼图输出从 `/tmp/bjx_sheets` 改到 `$BJX_BASE/metrics/sheets/`，不再写 `/tmp`。
...
### 3.3 `pack.py`（路径与暂存调整，定位调整为备份工具）

- `BASE = os.environ.get("BJX_BASE") or os.path.expanduser(os.path.join("~", "bjx", "data"))`；
  `OUT = os.environ.get("BJX_OUT") or os.path.expanduser(os.path.join("~", "bjx", "backup"))`。
- zip 暂存从 `/tmp` 改到 `$BJX_OUT` 内（`.tmp` 后缀 + `os.replace` 原子落位），`shutil` 依赖随之移除。
...
- 因 BASE 默认值改为 `~/bjx/data`，做了保持沙箱流程可用的兼容性改动，不涉及逻辑变化：`bootstrap.sh` 开头导出 `BJX_BASE`（默认仍为沙箱路径），且 `run` 模式不再经 `/tmp` 拷贝、直接原地运行；`daily_task.md` Step 0 补充环境变量说明、各 Step 命令同步为原地运行；`SKILL.md` 映射表与 `FORMAT.md` 目录清单登记新增的 `site.py`、`run_daily.sh`。
...
2. **`site.py`（核心新增，约 150 行）**——静态站点生成器：
   ...
   - 用 `markdown` 库（`extensions=["tables"]`）把每份简报渲染为 `site/report/YYYY-MM-DD.html`（套用统一 HTML 模板 + `assets/style.css`，简报内的原文链接保持外链到北极星）；
   - 生成 `site/index.html`：报告按日期倒序列表，每条展示日期、收录篇数、待补抓数（从简报的引用行正则解析）；顶部横幅展示最近一次抓取时间与成功/失败状态（读取最新日志是否含 `=== 完成` 行判断），便于一眼巡检；
   - 幂等：每次全量重渲染，秒级完成。
   - 不渲染文章全文页（简报已含原文外链）。如需站内全文阅读，二期再扩展 `articles/` 渲染 + 图片复制，静态生成成本同样很低。

3. **`run_daily.sh`**——每日流程编排（systemd 用户级 service 的 ExecStart）：
   ...
   PY=./.venv/bin/python
   [ -x "$PY" ] || PY=./.venv/Scripts/python    # Windows 风格 venv 布局
   [ -x "$PY" ] || PY=python3
   "$PY" crawler.py run        # 内部自动 xvfb-run
   "$PY" site.py
   # 可选: "$PY" pack.py export
   ```
...
不需要创建专用用户，也不需要 sudo 建目录——一切在登录用户家目录下进行。
...
```bash
# 本地（开发机）执行: 按部署结构直接上传到服务器 ~/bjx/(rsync 远程相对路径即家目录)
...
sudo .venv/bin/playwright install-deps chromium   # 装 Chromium 系统级依赖(仅这步需 sudo)
```
...
预期（对照 REBUILD.md 第4步）：20 个栏目输出增量统计，详情成功率接近 100%，生成 `~/bjx/data/briefing/$(date +%F).md`。
...
验证通过后手动跑一次站点生成：`cd ~/bjx && .venv/bin/python site.py`，确认 `~/bjx/site/index.html` 产出。

### 4.6 Nginx + HTTPS

`deploy/bjx.nginx.conf` 模板中站点根目录为 `/home/<USER>/bjx/site`，安装时替换为实际用户名：

```bash
chmod o+x "$HOME"     # nginx(www-data) 需能进入用户目录; 文件默认 umask 022 即可读
...
### 4.7 定时任务（systemd 用户级 timer）

unit 文件（`deploy/bjx-daily.service` / `bjx-daily.timer`）安装到用户级目录，无需 root：
...
service 中 `ExecStart=%h/bjx/run_daily.sh`，`%h` 自动解析为当前用户家目录；`BJX_BASE`/`BJX_SITE` 由 run_daily.sh 按 `$HOME/bjx/*` 默认值解析，无需在 unit 里写死路径。

### 4.8 端到端验收

```bash
systemctl --user start bjx-daily.service            # 手动触发一次完整流程
journalctl --user -u bjx-daily.service -e           # 查日志
...
| 站点改版 | 北极星改页面结构会导致解析为空，`crawler.py` 日志有 `解析为空告警`；巡检看首页"最近运行状态"横幅 + `~/bjx/data/logs/`。修复后格式按 FORMAT.md 演进。 |
...
| 备份 | 真正不可再生的状态只有 `~/bjx/data/state/seen.json` 与 `~/bjx/data/articles/`；用 `pack.py export`（导出到 `~/bjx/backup`）+ rsync/COS 做周期备份。 |
