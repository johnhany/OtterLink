...
4. `## 市场数值`：优先采用图片表格转录成果（由 state/briefing_manual.md 注入；无人工内容时回退为自动抽取——取含数值的完整语句：按句读标点 `。！？；` 扩展、超长收缩到分句 `，、：` 再硬截断；片段为标题或其子串时因无增量信息而跳过）。同质数据一律用 Markdown 表格展示，**正文中不得出现 MANUAL:START/END 等注释标记**
