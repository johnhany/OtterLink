...
- 环境变量：脚本路径基准已参数化为 `BJX_BASE`（默认 `/opt/bjx/data`，服务器部署见 design.md）。沙箱内手动执行 Step 1–5 前先 `export BJX_BASE=/mnt/agents/work/bjx_daily`（`bootstrap.sh` 已自动导出）；
- work 目录在 → 校验后直接就绪；
...
nohup python3 /tmp/crawler.py run > /tmp/crawler_run.out 2>&1 &
```
...
```bash
python3 /tmp/crawler.py mark --ids <ID列表>
```
...
得到 `/tmp/bjx_sheets/sheet_XX.jpg` 索引拼图（候选图已按 OCR 预筛压缩）。逐张读拼图：
